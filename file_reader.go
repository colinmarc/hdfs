...
// A FileReaderImpl represents an existing file or directory in HDFS. It implements
// io.Reader, io.ReaderAt, io.Seeker, and io.Closer, and can only be used for
// reads. For writes, see FileWriterImpl and ClientImpl.Create.
...
func (c *ClientImpl) Open(name string) (FileReader, error) {
...
// Stat returns the FileInfo structure describing file.
func (f *FileReaderImpl) Stat() os.FileInfo {
	return f.info
}

// SetDeadline sets the deadline for future Read, ReadAt, and Checksum calls. A
// zero value for t means those calls will not time out.
func (f *FileReaderImpl) SetDeadline(t time.Time) error {
	f.deadline = t
...
// are stored alongside the data) for each block, and then calculating the MD5
// of all of those.
func (f *FileReaderImpl) Checksum() ([]byte, error) {
	if f.info.IsDir() {
...
//
// The seek is virtual - it starts a new block read at the new position.
func (f *FileReaderImpl) Seek(offset int64, whence int) (int64, error) {
	if f.closed {
...
// Read implements io.Reader.
func (f *FileReaderImpl) Read(b []byte) (int, error) {
	if f.closed {
		return 0, io.ErrClosedPipe
	}
...
// ReadAt implements io.ReaderAt.
func (f *FileReaderImpl) ReadAt(b []byte, off int64) (int, error) {
	if f.closed {
		return 0, io.ErrClosedPipe
	}
...
// That's because HDFS has no mechanism for limiting the number of entries
// returned; whatever extra entries it returns are simply thrown away.
func (f *FileReaderImpl) Readdir(n int) ([]os.FileInfo, error) {
	if f.closed {
...
func (f *FileReaderImpl) readdir() ([]os.FileInfo, int, error) {
...
// error before the end of the directory, Readdirnames returns the names read
// until that point and a non-nil error.
func (f *FileReaderImpl) Readdirnames(n int) ([]string, error) {
	if f.closed {
		return nil, io.ErrClosedPipe
	}
...
func (f *FileReaderImpl) getBlocks() error {
...
func (f *FileReaderImpl) getNewBlockReader() error {
	off := uint64(f.offset)
	for _, block := range f.blocks {
